...
func test_goal_detected_left_side() -> void:
	var result := gd.check_goal(Vector2(-1.0, 120.0), 0.0, true)
	assert_true(result["is_goal"], "Ball past left goal line, between posts, on ground")
	assert_eq(result["side"], "left")


func test_goal_detected_right_side() -> void:
	var result := gd.check_goal(Vector2(321.0, 120.0), 0.0, true)
	assert_true(result["is_goal"], "Ball past right goal line, between posts, on ground")
	assert_eq(result["side"], "right")


func test_no_goal_above_crossbar() -> void:
	var result := gd.check_goal(Vector2(-1.0, 120.0), 10.0, true)
	assert_false(result["is_goal"],
		"Ball above crossbar height should not be a goal")


func test_no_goal_outside_posts_top() -> void:
	var result := gd.check_goal(Vector2(-1.0, 90.0), 0.0, true)
	assert_false(result["is_goal"],
		"Ball above top post should not be a goal")


func test_no_goal_outside_posts_bottom() -> void:
	var result := gd.check_goal(Vector2(-1.0, 150.0), 0.0, true)
	assert_false(result["is_goal"],
		"Ball below bottom post should not be a goal")


func test_no_goal_ball_not_in_play() -> void:
	var result := gd.check_goal(Vector2(-1.0, 120.0), 0.0, false)
	assert_false(result["is_goal"],
		"Ball not in play should not be a goal")


func test_no_goal_ball_on_pitch() -> void:
	var result := gd.check_goal(Vector2(160.0, 120.0), 0.0, true)
	assert_false(result["is_goal"],
...
func test_goal_at_exactly_crossbar_height() -> void:
	var result := gd.check_goal(
		Vector2(-1.0, 120.0), GoalDetectionPure.CROSSBAR_HEIGHT, true)
	assert_true(result["is_goal"],
		"Ball at exactly crossbar height should be a goal (<=)")


func test_goal_at_top_post_boundary() -> void:
	var result := gd.check_goal(
		Vector2(-1.0, GoalDetectionPure.GOAL_MOUTH_TOP), 0.0, true)
	assert_true(result["is_goal"],
		"Ball at exactly top post Y should be a goal")


func test_goal_at_bottom_post_boundary() -> void:
	var result := gd.check_goal(
		Vector2(-1.0, GoalDetectionPure.GOAL_MOUTH_BOTTOM), 0.0, true)
	assert_true(result["is_goal"],
		"Ball at exactly bottom post Y should be a goal")


func test_goal_at_exactly_goal_line_left() -> void:
	var result := gd.check_goal(
		Vector2(GoalDetectionPure.GOAL_LEFT_X, 120.0), 0.0, true)
	assert_true(result["is_goal"],
		"Ball at exactly goal line X should be a goal")


func test_goal_at_exactly_goal_line_right() -> void:
	var result := gd.check_goal(
		Vector2(GoalDetectionPure.GOAL_RIGHT_X, 120.0), 0.0, true)
	assert_true(result["is_goal"],
		"Ball at exactly goal line X should be a goal")
...
func test_in_goal_mouth_center() -> void:
	assert_true(gd.is_in_goal_mouth(120.0))


func test_not_in_goal_mouth_above() -> void:
	assert_false(gd.is_in_goal_mouth(95.0))


func test_not_in_goal_mouth_below() -> void:
	assert_false(gd.is_in_goal_mouth(145.0))
...
func test_left_goal_scores_for_away() -> void:
	ms.start_play()
	ms.record_goal("left")
...
func test_right_goal_scores_for_home() -> void:
	ms.start_play()
	ms.record_goal("right")
